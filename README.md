...
| **주파수 탐색기 (Modal)** | **실시간 스펙트럼 분석(PSD)**을 통해 지배적 주파수를 자동으로 찾고, 클릭 한 번으로 분석 주파수를 설정하는 모달 팝업 도구 |
| **워크플로우 제어** | 파일 업로드 후 타겟 주파수를 확정하고 **`▶️ 분석 시작`** 버튼을 누르는 명확한 분석 프로세스 제공 |
...
| **가독성 최적화** | 가동 판정 기준을 **백분율(%)**로 표시하여 그래프 상의 높이와 직관적으로 매칭 |
...
│   │   └── analysis.py # 신호 처리 파이프라인 (V5.7 로직 포함)
│   ├── ui/             # 사용자 인터페이스 (View Layer)
│   │   ├── layout.py   # 메인 레이아웃 및 워크플로우 제어
│   │   ├── plots.py    # 시각화 컴포넌트 (MM:SS 포맷)
│   │   ├── analyzer.py # 주파수 탐색기 (Modal Dialog)
│   │   └── timeline.py # 가동 상태 타임라인 시각화
│   └── main.py         # 애플리케이션 진입점
├── docs/               # 문서 (가이드, 진행상황)
